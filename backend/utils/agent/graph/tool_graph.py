...
def build_tool_graph() -> StateGraph:
    # 0. 问题改写节点:结合记忆补全用户问题
    from utils.agent.node.tool.question_rewrite import question_rewrite_node

    # 1. 意图识别节点:分析并标记需要的工具类型
    from utils.agent.node.tool.intent_classifier import intent_classifier_node

    # 2. 后端工具节点:根据标记决定是否执行
    from utils.agent.node.tool.backend_tools import backend_tools_node

    # 2.1 路由函数:决定是否执行后端工具
    from utils.agent.route.tool.backend_tools import backend_tools_route

    # 2.2 后端工具执行节点
    # Send 到这个节点,分为并行(toolnode)(自动执行)和 (serialtoolnode)(人类参与)
    from utils.agent.node.tool.backend_tool_execution import (
        backend_tool_execution_node,
    )

    # 2.3 后端工具检查节点
    from utils.agent.node.tool.backend_tool_check import (
        backend_tool_check_node,
    )

    # 2.4 后端检查点路由
    from utils.agent.route.tool.backend_tool_check import (
        backend_tool_check_route,
    )

    # 3. 前端工具节点:根据标记决定是否生成工具调用
    from utils.agent.node.tool.frontend_tools import frontend_tools_node

    # 3.1 路由函数:决定是否执行前端工具
    from utils.agent.route.tool.frontend_tools import frontend_tools_route

    # 3.2 前端工具执行节点
    from utils.agent.node.tool.frontend_tool_execution import (
